...
export class MainMenu extends BaseScene {
    ...
    create (): void {
        ...
        this.title = this.add.text(0, 0, 'Main Menu', {
            fontFamily: 'Verdana, sans-serif', fontSize: 38, color: '#ffffff',
    ...
    override refreshLayout (): void {
        ...
        if (scaleUtils.isPortrait()) {
            // Портрет
            this.logo.setPosition(this.centerX, scaleUtils.getRelativeY(200))
                     .setScale(this._scale * 1.6);

            this.title.setPosition(this.centerX, scaleUtils.getRelativeY(300))
                      .setFontSize(128 * this._scale);
        } 
        else {
            // Ландшафт
            this.logo.setPosition(scaleUtils.currentWidth * 0.4, this.centerY)
                     .setScale(this._scale);

            this.title.setPosition(scaleUtils.currentWidth * 0.7, this.centerY)
